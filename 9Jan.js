...
//     let restaurants = await dbHotel.find({ borough: 'Bronx' }).sort({ name: -1 });
//     return restaurants
...
// let display = async() => {
//     let restaurants = await dbHotel.find({ borough: 'Bronx' }).skip(5).limit(5)
//     return restaurants
...
//     let restaurants = await dbHotel.find({ cuisine: { $nin: ['American'] }, 'grades.grade': 'A', borough: { $nin: ['Brooklyn'] } }).sort({ cuisine: -1 })
//     return restaurants
...
let display = async() => {
    ...
    let restaurants = await dbHotel.aggregate([
        { $match: { cuisine: { $in: ["Bakery", "Italian"] }, borough: "Bronx" } },
...
//         let restaurants = await dbHotel.find({ name: /mon/i }, { "name": 1, "borough": 1, "address.coord": 1, cuisine: 1 }).limit(100)
//         return restaurants
...
// let restaurants = dbHotel.aggregate([{
//         $project: {
//             _id: 0,
//             name: 1,
//             restaurant_id: 1,
//             increasedScore: { $add: ['$grades.score', 2] }
//         }
//     }], (err, data) => {
...
// display(console.log('---------restaurants--------', display(), '--------restaurants-------'))
